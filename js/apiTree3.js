...
function Triangle(p0,height) {
    this.p0 = p0;
    this.height = height;
}

Triangle.prototype.render = function (context) {
    drawUtil.triangle(context,this.p0,this.height);
};
...
Curve.prototype.render = function (context) {
    drawUtil.bezierCurve(
        context,
        this.p0,
        {x:this.p0.x,y:this.p1.y},
        {x:this.p1.x,y:this.p0.y},
        this.p1
    );
};
...
ApiTree.prototype.getLayerNum = function (node, currentLayer = 1) {
    ...
    if (parentNode) {
        ...
        return this.getLayerNum(parentNode, currentLayer + 1);
    } else {
        return currentLayer;
    }
};
    ...
    for (let i = 0; i < this.freeNodes.length; i++) {
        ...
        for (let j = 0; j < layer.length; j++) {
            ...
            if(node.children.length){
                node.triangle = new Triangle({x:node.x,y:node.y+node.radius+node.lineWidth},this.triangleHeight);
            }
            if (i !== 0) {
            let parentNode = node.extend;
